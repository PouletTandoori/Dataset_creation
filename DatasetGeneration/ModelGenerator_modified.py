...
class ModelGenerator:
    ...
    def generate_model(self, stratigraphy=None, thicks=None, dips=None,
        ...
        if self.Halt_seq is None:
            ...
            # transform Halton sequence into dictionary, each coloumn is a key:
            self.Halt_seq = {keys[i]: Halton_seq[:, i] for i in range(len(keys))}
            # print('Halton sequence:',Halton_seq[:20])
            Halt_seq = self.Halt_seq
        else:
            print('[ModelGenerator] Halton seq exists, nlayer_max=', self.layer_num_max)
            Halt_seq = self.Halt_seq

        if thicks is None:
            if boundaries is None:
                thicks,self.nlayer = random_thicks(self.NZ,
                                       self.layer_dh_min,
                                       self.layer_dh_max,
                                       self.layer_num_min,
                                       self.layer_num_max,
                ...
                self.nlayer=len(thicks)
        print('nlayer = ', self.nlayer, '=', len(thicks))

        if stratigraphy is None:
            stratigraphy = self.stratigraphy

        else:
            print('stratigraphy is not None')
        ...
        layers = stratigraphy.build_stratigraphy(thicks, dips)
        if boundaries is None:
        ...
        # remove 1 row from key 'Nb_layers' and nlayers rows from  the other keys:
        Halt_seq['Nb_layers']=Halt_seq['Nb_layers'][1:]
        Halt_seq['Ordered']=Halt_seq['Ordered'][1:]
        Halt_seq['thickness']=Halt_seq['thickness'][self.nlayer:]
        Halt_seq['Vp']=Halt_seq['Vp'][self.nlayer:]
        Halt_seq['Vs']=Halt_seq['Vs'][self.nlayer:]
        Halt_seq['Q']=Halt_seq['Q'][self.nlayer:]
        Halt_seq['litho']=Halt_seq['litho'][self.nlayer:]

        #créer un tuple de sorties : propdict, layerids, layers, Halt_seq_reduced
        outputs= (propdict, layerids, layers, Halt_seq)
...
class Stratigraphy(object):
    ...
    def __init__(self, sequences=None, defaultprops=None, Halton_seq=None):
        ...
        if sequences is None:
            ...
            sequences = [Sequence(lithologies=[litho], Halton_seq=Halton_seq)]
    ...
    def build_stratigraphy(self, thicks, dips):
        ...
        for ii, (t, di) in enumerate(zip(thicks, dips)):
            ...
            # pour une ligne de Halton, on a 3 valeurs:(Vp, Vs, Q) * nombre de couches dans le modele
            #1 modèle = 1 ligne de Halton, mais on parcour la ligne
            jj=0
            for prop in lith:
                #give the name of the property:
                if prop.name=='vp':
                    #random value:
                    val=prop.min + Halton_seq['Vp'][jj] * (prop.max - prop.min)
                    vp=val
                    jj+=1
                elif prop.name=='vs':
                    #random value:
                    val=prop.min + Halton_seq['Vs'][jj] * (prop.max - prop.min)
                    vs=val
                    #print('layer',ii,'Vp=',vp,'Vs=',vs)
                    jj+=1
                elif prop.name=='vpvs':
                    #dot not augment jj:
                    ...
                    #use Gardner law: rho = 0.31 * V_p^{0.25}
                    val= 0.31 * vp**0.25
                    #but stay in the range of the property:
                    if val<prop.min:
                        val=prop.min
                    if val>prop.max:
                        val=prop.max
                    ...
                    #random value:
                    val=prop.min + Halton_seq['Q'][jj] * (prop.max - prop.min)
                    jj+=1
                else:
                    print(f'Property {prop.name} not found')

                properties.append(val)

                #print(f'Layer {ii}, Property {prop.name}, Value: {val}')
        ...
        #Premièrement trier les couches afin de les séparer en deux groupes selon la lithologie:
        #Premier groupe : couches avec lithologies comprises dans ['Organic soils','Dry Sands', 'Wet sands', 'Silts', 'Clay']
        #Deuxième groupe : couches avec lithologies comprises dans [''Tills','Sandstone','Dolomite','Limestone','Shale']

        # Pour ce faire, faire en sorte que les premières couches soient du premier groupe et ensuite viennent les couches du deuxieme groupe
        # Définir les deux groupes lithologiques
        group1 = ['Organic soils', 'Dry Sands', 'Wet sands', 'Silts', 'Clay']
        group2 = ['Tills', 'Sandstone', 'Dolomite', 'Limestone', 'Shale']

        # Séparer les couches selon le groupe lithologique
        layers_groupe1 = [layer for layer in layers if layer.lithology.name in group1]
        layers_groupe1 = sorted(layers_groupe1, key=lambda x: (x.vp, x.vs))
        # si Halton_seq['Ordered'][0] < 0.15, intervertir 2 couches voisines:
        if Halton_seq['Ordered'][0] < 0.15:
            for i in range(len(layers_groupe1)-1):
                if layers_groupe1[i].vp > layers_groupe1[i+1].vp:
                    layers_groupe1[i], layers_groupe1[i+1] = layers_groupe1[i+1], layers_groupe1[i]



        layers_groupe2 = [layer for layer in layers if layer.lithology.name in group2]
        layers_groupe2 = sorted(layers_groupe2, key=lambda x: (x.vp, x.vs))

        if Halton_seq['Ordered'][0] < 0.15:
            for i in range(len(layers_groupe2)-1):
                if layers_groupe2[i].vp > layers_groupe2[i+1].vp:
                    layers_groupe2[i], layers_groupe2[i+1] = layers_groupe2[i+1], layers_groupe2[i]

        # Vérifier si une couche n'appartient à aucun groupe
        layers_autres = [layer for layer in layers if layer.lithology.name not in group1 + group2]
        if layers_autres:
            print("⚠️ Attention : certaines lithologies n'appartiennent à aucun groupe :")
            for l in layers_autres:
                print(f"- {l.lith.name}")

        # Concaténer les deux groupes : Groupe 1 puis Groupe 2
        layers = layers_groupe1 + layers_groupe2 + layers_autres
...
class Sequence(object):
    ...
    def __init__(self, name="Default", lithologies=None, ordered=False,
                 ...
                 nmin=1, deform=None, skip_prob=0, accept_decrease=1,halton_seq=None,nlayers=None):
        """
        ...
        :param ordered: If True, the Sequence will be classified according to Vp and Vs.
    ...
    def __next__(self):
        if self.n < self.nmax:
            Halt_value = self.Halton_seq['litho'][self.n]
            Nb_lith = len(self.lithologies)
            # print('Nombre de lithologies:',Nb_lith)
            proportions = 1 / Nb_lith
            integers = np.arange(0, Nb_lith)
            # print('integers:',integers)

            for i in integers:
                if Halt_value < proportions * (i + 1) and Halt_value > proportions * i:
                    out = self.lithologies[i]
                    break
            self.n += 1

            return out
        else:
            print('n=', self.n,'et nmax=',self.nmax)
            raise StopIteration
...
def random_thicks(nz, thickmin, thickmax, nmin,nmax, nlayer,
    ...
    if Halton_seq is None:
        raise  ValueError("Halton sequence is not defined")

    #thickmax = min(int(nz / nmin), thickmax)
    if thickmax < thickmin:
        print("⚠️ Warning: épaisseur max < épaisseur min. Correction automatique.")

    halton_nlayer = Halton_seq['Nb_layers'][0]
    #diff min et maxlayers:
    diff= nmax - nmin
    #halton entre 0 et 1
    nlayer = math.ceil(halton_nlayer * (diff-1)) + 1 + nmin
    nlayer = int(nlayer)

    #print('nlayer:', nlayer)

    #step 1: generate the thicknesses of the layers
    thicks = np.zeros(nlayer)

    # Dans la séquence de Halton pour la key 'thickness' o n prend nlayer valeurs:
    halton_ticks = Halton_seq['thickness'][0:nlayer]

    # on calcule les épaisseurs correspondantes:
    for i in range(nlayer):
        thicks[i] = thickmin + halton_ticks[i] * (thickmax - thickmin)

    # On normalise les épaisseurs pour que la profondeur atteinte soit égale à la profondeur maximale.
    Prof_atteinte = np.sum(thicks)
...
def gridded_model(nx, nz, layers, lz, lx, corr):
    ...
    for layer in layers[1:]:
        ...
        for jj, z in enumerate(layer.boundary):
            ...
            if addtrend is not None:
                for n in range(npar):
                    props2d[n][z:, jj] = props2d[n][z:, jj].astype(np.float64) + (trends[n] * np.arange(z, nz))
