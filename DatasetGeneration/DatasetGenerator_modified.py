...
class DatasetGenerator:
    ...
    def read(self, filename: str, toread: list = None):
        ...
        with h5.File(filename, "r") as file:
            print('les keys présentes dans le fichier:', file.keys())
            if toread is None:
            ...
            for key in toread:
                print('key:', key)
                if key == 'DispersionCurves':
                    #print('DispersionCurves')
    ...
    def run(self):
        ...
        with tqdm(total=total_seeds, desc="Generating dataset",colour='green') as pbar:
            ...
            while not self.seeds.empty():
                ...
                # print le temps de création de l'exemple
                t0= time()
                ...
                pbar.update(1)
                tf= time() - t0
                print(f"Example {seed} generated in {tf:.2f} seconds.")
            pbar.close()
